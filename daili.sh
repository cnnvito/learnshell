...
#删除脚本配置
	...
	#rm -f /etc/environment.stop /etc/environment.start
	source /etc/environment
...
echo
echo "
***开启代理前，请在/etc/shadowsocksr/config.json填好你的\$\$r节点信息。
...
python /usr/local/shadowsocksr/shadowsocks/local.py -c /etc/shadowsocksr/config.json -d start
#全局代理写入环境变量
cp -a /etc/environment /etc/environment.bak 
#cp -a /etc/environment /etc/environment.stop
...
脚本发布于 愉悦人生 https://www.cnvito.top

要是脚本对你有帮助，在我的博客下面留个言可好？
